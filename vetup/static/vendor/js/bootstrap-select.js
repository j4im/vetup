    ...
    clickListener: function () {
      ...
      this.$menuInner.on('click', 'li a', function (e, retainActive) {
        ...
        if (!that.isDisabled() && !$this.parent().hasClass(classNames.DISABLED)) {
		  ...
		  var rangeSelect = that.options.rangeSelect;
		  if (rangeSelect && !state) {
			  var n_selected = 0
			  var selected_ix
			  for(i=0; i < that.selectpicker.main.data.length; i++) {
				  if (that.selectpicker.main.data[i].selected) {
					  n_selected++
					  selected_ix = i
				  }
			  }
			  if (n_selected===1) {
				  for (i=Math.min(selected_ix, clickedIndex)+1; i <= Math.max(selected_ix, clickedIndex); i++) {
					  if (i !== clickedIndex) {
						  that.selectpicker.main.data[i].option.selected = true
